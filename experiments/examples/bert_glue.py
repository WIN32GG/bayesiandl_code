...
def sample_bayesian(
    ...
    log_prior                 = torch.zeros(samples                      ).to(device)
    log_variational_posterior = torch.zeros(samples                      ).to(device)
...
def train(EXP: str, MODEL_NAME: str, TASK_NAME: str, N_LABELS: int, DELTA: float, WEIGHT_DECAY: float, DEVICE: str) -> float:
    ...
    N_WARMUP_STEPS = 0
    MAX_GRAD_NORM  = 1
    DATA_DIR       = os.path.join("./dataset/glue_data", TASK_NAME)

    dumper = Dumper("dump_glue/glueexp")
    ...
    with dumper("frequentist_test"):
        with torch.no_grad():
            ...
            for inputs in pbar:
                ...
                with dumper():
                    dumper['labels'] = labels.view(-1)
                    dumper['logits'] = logits.view(-1, N_LABELS)
    ...
    with dumper("bayesian_eval_after_train"):
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    train("exp", "distilbert-base-uncased", "MRPC", args.n_labels, 1e-1, 1e-3, args.device)
