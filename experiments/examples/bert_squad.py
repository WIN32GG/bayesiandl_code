...
def setup_fake_dataset(data_dir: str, tokenizer: nn.Module, test: bool = False, **kwargs) -> Tuple[Dataset, torch.Tensor, torch.Tensor]:
    test = False
    cached_path = os.path.join(data_dir, f"fakev1.pth")#os.path.join(data_dir, f"{'dev' if test else 'train'}v1.pth")
    if os.path.isfile(cached_path):
        ckpt = torch.load(cached_path)
        return ckpt["dataset"], ckpt["examples"], ckpt["features"]
    
    processor   = SquadV1Processor()
    fname       = "dev-v1.1.json"#f"{'dev' if test else 'train'}-v1.1.json"
...
def load_freq(o_model):
    o_model.load_state_dict(torch.load('o_model.pth').module.state_dict())
    
def load_baye(b_model):
    b_model.load_state_dict(torch.load('b_model.pth'))
...
def train(EXP: str, MODEL_NAME: str, DELTA: float, WEIGHT_DECAY: float, DEVICE: str) -> float:
    ...
    fake_dataset,  fake_examples,  fake_features  = setup_fake_dataset(DATA_DIR,    tokenizer=tokenizer, test=False, **squadv1)

#     print(len(fake_features))
#     print(len(test_features))
#     print(len(train_features))
#     print(set([x for x in fid if fid.count(x) > 1]))
#     print(fid.count("7wo24e4mrm9o3kuqd2u9bd45"))
#     print(tid.count("7wo24e4mrm9o3kuqd2u9bd45"))
#     q = test_features[151919].qas_id
#     print(tid.count(q))
#     print(fid.count(q))
    
#     print(dir(test_features[0]))
#     exit(0)
    ...
    s_model = copy.deepcopy(o_model)
            
    with dumper("frequentist_train2"):
        for epoch in tqdm(range(EPOCHS), desc="Epoch"):
...
#             writer.add_scalar("test_total", report.total, epoch)
    # ============================ EVALUTATION ====================================
    b_model = to_bayesian(s_model, delta=DELTA, freeze=FREEZE)
    load_baye(b_model)
...
#                     with dumper():
#                         dumper['start_position']           = start_positions                                                                                                        
#                         dumper['end_position']             = end_positions
#                         dumper['start_logits']              = raw_start_logits
#                         dumper['end_logits']                = raw_end_logits
...
if __name__ == "__main__":
    ...
    print(train(args.exp, args.model_name, 1e-1, 1e-2, args.device))
