...
CENTERED_REDUCED = Normal(torch.tensor(0.).cuda(), torch.tensor(1.).cuda())
...
class Linear(Module):
    ...
    def from_frequentist(
        ...
        if delta is not None:
            ...
            # MOPED Version: mu from weights, rho to 1
            # prior.mu.data.copy_(linear.weight.data)
            # prior.rho.data = torch.ones_like(linear.weight) * UNIT_RHO

            # BAM Version: Initialize at random
            prior.mu.data.copy_(CENTERED_REDUCED.sample(prior.mu.shape))
            prior.rho.data.copy_(CENTERED_REDUCED.sample(prior.rho.shape))
