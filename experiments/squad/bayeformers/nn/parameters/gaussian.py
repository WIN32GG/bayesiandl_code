...
F32_ε  = 1.1920929e-07
M_LOG_SQRT_2PI = - np.log(np.sqrt(2 * np.pi))
...
class Gaussian(Parameter):
    ...
    def log_prob(self, input: Tensor) -> Tensor:
        ...
        return (
            M_LOG_SQRT_2PI
            - torch.log(self.sigma + F32_ε)
            - ((input - self.mu) ** 2) / (2 * self.sigma ** 2 + F32_ε)
        ).sum()
...
class ScaledGaussianMixture(Parameter):
    ...
    def __init__(self, pi: float, sigma1: float, sigma2: float) -> None:
        ...
        self.register_parameter("pi",     nn.Parameter(torch.tensor(pi,     dtype=torch.float32), requires_grad = False))
        self.register_parameter("sigma1", nn.Parameter(torch.tensor(sigma1, dtype=torch.float32), requires_grad = False))
        self.register_parameter("sigma2", nn.Parameter(torch.tensor(sigma2, dtype=torch.float32), requires_grad = False))
        self.register_parameter("zero",   nn.Parameter(torch.tensor(0.,     dtype=torch.float32), requires_grad = False))
    ...
    def log_prob(self, input: Tensor) -> Tensor:
        ...
        normal_logprob = lambda loc, scale, value: -((value - loc) ** 2) / (2 * (scale ** 2)) - scale.log() - math.log(math.sqrt(2 * math.pi))

        prob1 = torch.exp(normal_logprob(self.zero, self.sigma1, input))
        prob2 = torch.exp(normal_logprob(self.zero, self.sigma2, input))
